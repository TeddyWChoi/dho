...
$(function(){
	...
	*/
	...
	$('.siteMap').on('click',function(){
		$(this).parent().next().slideToggle(300,function(){
			...
			if($(this).attr('class') != "site_wrap on"){
				$(this).addClass('on')
				$(this).parent().find('.siteMap span').css({'background-position':'142px -992px'});
				tf=false;
				
			}else{
				
				$(this).removeClass('on')
				$(this).parent().find('.siteMap span').css({'background-position':'142px -961px'});
				tf=true;
	...
	/*배너롤링*/
	var bannerArry = [
		{src:'/pb/fr/img/com/temp/img_banner_temp.png', alt:'기획확인 요망1', dataLink: '#'},
	...
	$.fn.slideCol1 = function(data){
		...
		function cfFunc(thisVar)
		{
			...
			if(n2 != thisVar)
			{
				...
				if(btn_tf == true) // 연속클릭방지
				{
					...
					$wrapList.find(".bn_area dt a").not(".on").parent().next().css({"opacity":alphaNum,"left":leftNum2,'top':topNum2, 'z-index':'0'}); //초기 셋팅 
					$wrapList.find(".bn_area .on").parent().next().stop().animate({"opacity":alphaNum,'left':leftNum1,'top':topNum1}, aniTime ,function(){ 
					...
					$wrapList.find(".bn_area dt").eq(thisVar).next().stop().animate({"opacity":"1",'left':'0','top':'0'}, aniTime ,function(){
		...
		$wrapList.find(".ctrl_btn .ps_btn").on("click",function(){
			if(psNum == true)
			{
				...
				$wrapList.find('.ps_btn').css({'background':'url(/pb/fr/img/com/imgSlider_zip.png) 0px -34px no-repeat'})/*09.28 최웅 수정*/
			...
			else if(psNum == false)
			{
				...
				psNum = true;
				$wrapList.find('.ps_btn').css({'background':'url(/pb/fr/img/com/imgSlider_zip.png) 0px -53px no-repeat'})/*09.28 최웅 수정*/
