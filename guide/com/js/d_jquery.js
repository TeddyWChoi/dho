...
**/
/** gnbBox **/
var gnbBox_animate = "N"; // 애니메이션 사용여부 ( Y, N )
function gnbBox_init(cnt){
	//기본
	gnbBox_set(cnt);
	...
	jQuery("#gnbBox").hover(
		...
		function(){
			setTimeout(function(){gnbBox_set(cnt)},200);
		}
	);
	jQuery("#gnbBox > ul > li > a").hover(function(){gnbBox_ov(this)},function(){return false});
	...
	jQuery("#gnbBox > ul > li > a").focus(function(){gnbBox_ov(this)});
	
}
function gnbBox_set(cnt){
	jQuery("#gnbBox_ov").stop();
	cnt--;
	var point_href = jQuery("#gnbBox > ul > li").eq(cnt).find(" > a").attr("href");
	var point_text = jQuery("#gnbBox > ul > li").eq(cnt).find(" > a").text();
	var point_width = jQuery("#gnbBox > ul > li").eq(cnt).width();
	var point_left = jQuery("#gnbBox > ul > li").eq(cnt).position().left;
	
	if(navigator.appVersion.indexOf("MSIE 6")!=-1){
		//IE6.0용 코드
		var point_top = jQuery("#gnbBox > ul > li").eq(cnt).position().top + jQuery("#gnbBox > ul > li").height();	
	} else {	
		//그외 코드
		var point_top = jQuery("#gnbBox > ul > li").eq(cnt).position().top;	
	}


	
	jQuery("#gnbBox_ov").html('<strong><a href="'+point_href+'">'+point_text+'</a></strong>');
	jQuery("#gnbBox_ov").width(point_width);
	if(gnbBox_animate == "Y"){
		jQuery("#gnbBox_ov").animate(
			{top:point_top}
			,1
			,function(){
				jQuery("#gnbBox_ov").animate(
		...
		jQuery("#gnbBox_ov").css({"left":point_left+"px","top":14+"px"});
	}
}
function gnbBox_ov(hit){
	var hitCount = jQuery("#gnbBox > ul > li > a").index(jQuery(hit))+1;
	//실행
	gnbBox_set(hitCount);
}
...
var lm_height;
var lm_speed = 300;
function lnb_menu(lm1,lm2){
	//3차메뉴 기본 숨기기
	jQuery("#lnb > li > ul").hide();
	jQuery("#lnb > li > ul").animate({opacity:0});	
	for(i=0; i < jQuery("#lnb > li").length; i++)
	{
		jQuery("#lnb > li").eq(i).find(" > ul >li:last").css({"padding-bottom":"0px"});
	}
	//2차메뉴의 높이정의 및 기본높이저장
	//jQuery("#lnb > li").css({"height":li_normal_h+"px","overflow":"hidden"});
	lm_height = jQuery("#lnb > li").innerHeight();
	
	//2차메뉴 클릭시
	jQuery("#lnb > li > a").click(function(){
		var lm_2th_check = jQuery(this).parent().innerHeight();
...
function lm_3th_show(data){	
	...
	jQuery(data).parent().height(jQuery("#lnb > li > a").innerHeight());
	
	//3차메뉴 높이늘이기	
	if(jQuery(data).parent().find(">ul").length > 0){
		jQuery(data).parent().animate(
			{
				height:jQuery(data).parent().innerHeight()+jQuery(data).parent().find(">ul").innerHeight()
			},
			lm_speed
		);
		...
		jQuery(data).parent().animate(
			{
				height:jQuery(data).parent().innerHeight()
			},
			lm_speed
		);
	}
...
function lm_open(){
	...
	jQuery("#lnb > li > ul").animate({opacity:1});	
	jQuery("#lnb > li").height(jQuery("#lnb > li > a").innerHeight());
	...
	{		
		if(jQuery("#lnb > li").eq(i).find(">ul").length > 0)
		{
			jQuery("#lnb > li").eq(i).animate(
				{
					height:jQuery("#lnb > li").eq(i).innerHeight()+jQuery("#lnb > li").eq(i).find(">ul").innerHeight()
				},
				lm_speed
			);
			jQuery("#lnb > li").eq(i).find(">ul").animate(
				{
					height:jQuery("#lnb > li").eq(i).find(">ul").innerHeight()},lm_speed,
					function(){
			...
			jQuery("#lnb > li").eq(i).animate(
				{
					height:jQuery("#lnb > li").eq(i).innerHeight()
				},
				lm_speed
			);
		}
...
function lm_check(lm1,lm2){
	...
	jQuery("#lnb > li").eq(lm1).find(">a").toggleClass("link_1th_ov");
	jQuery("#lnb > li").eq(lm1).find("ul>li").eq(lm2).find(">a").toggleClass("link_2th_ov");	
}
...
$(document).ready(function(){
	$(".cordView").toggleSourceBox();
});

function disblePlugin(){
	
	$(".cordView").find(".btn a.btn_view").removeClass("on");
	$(".cordView").find(".btn a.btn_view strong").text("소스펼치기");
	$(".cordView").find(".btn a.btn_view strong").removeClass("on");
	$(".cordView").find(".tag").removeClass("close");
	$(".cordView").toggleSourceBox();

}

$.fn.toggleSourceBox = function(options){
	return this.each(function(){
			var self = $(this);
			var $button = self.find(".btn a.btn_view");
			var $source = self.find(".tag");
			
			var bAct = true;
			$button.click(function(){
				if(bAct){
						$(this).addClass("on");
						$(this).find("strong").text("code view");
						$(this).find("strong").addClass("on");
						$source.addClass("close");
						bAct = false;
				}else{
						$(this).removeClass("on");
						$(this).find("strong").text("code hide");
						$(this).find("strong").removeClass("on");
						$source.removeClass("close");
						bAct = true;
				}
...
function textCopy(idx) {
	var obj = document.getElementById('cordBox' + idx);
	var contents = document.body.createTextRange();
		contents.moveToElementText(obj);
		contents.execCommand('Copy');
		alert ('Ctrl+V 하시면 복사된 내용이 붙여넣기 됩니다.');
		}
